"""Utility functions for preprocessing data sets.对数据做预处理的操作们"""
...
def mean_image_addition(image, means=(_R_MEAN, _G_MEAN, _B_MEAN)):
  ...
  For example:
    ...
    image = _mean_image_addition(image, means)
  Note that the rank of `image` must be known.
  Args:
    image: a tensor of size [height, width, C].
    means: a C-vector of values to subtract from each channel.从每个channel减去
  Returns:
    the centered image.
  Raises:
...
def random_crop_or_pad_image_and_label(image, label, crop_height, crop_width, ignore_label):
  ...
  Resizes an image to a target width and height by rondomly 把一个图像重设置大小为想要的宽度和高度
  cropping the image or padding it evenly with zeros.均匀地修剪或补充0
  Args:
    image: 3-D Tensor of shape `[height, width, channels]`.
    label: 3-D Tensor of shape `[height, width, 1]`.
...
batch_size: The number of samples per batch. Need to be 1 for the images of different sizes.
